...
struct Solver 
{
    ...
    void straightly_make_rect_action() 
    {
        rep(x2, N) {
            rep(y2, N) {
                int t1_diff = 1, t2_diff = 1;
                int start_t1 = 1, end_t1 = MAX_DIAG + 1, start_t2 = 1, end_t2 = MAX_DIAG + 1;
                if (x2 % 3 == 0) {
                    t1_diff = -1;
                    t2_diff = -1;
                    start_t1 = MAX_DIAG;
                ...
                for (int t1 = start_t1; t1 != end_t1; t1 += t1_diff) {
                    for (int t2 = start_t2; t2 != end_t2; t2 += t2_diff) {
                        fore (t1_dx, dx) {
                            fore (t1_dy, dy) {
                                fore (t2_dx, dx) {
                                    fore (t2_dy, dy) {
