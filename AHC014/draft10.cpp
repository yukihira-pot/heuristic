...
struct Solver 
{
    ...
    void straightly_make_rect_action() 
    {
        rep(x2, N) {
            rep(y2, N) {
                if ( ( inc(x2, 2*N/5, 3*N/5+1) and inc(y2, 2*N/5, 3*N/5+1) ) ) {
                    rep(t1, 1, MAX_STRAIGHT + 1) {
                        rep(t2, 1, MAX_STRAIGHT + 1) {
                            fore (t1_dx, dx) {
                                fore (t1_dy, dy) {
                                    fore (t2_dx, dx) {
                                        fore (t2_dy, dy) {
                    ...
                    rrep(t1, MAX_STRAIGHT, 1) {
                        rrep(t2, MAX_STRAIGHT, 1) {
                            fore (t1_dx, dx) {
                                fore (t1_dy, dy) {
                                    fore (t2_dx, dx) {
                                        fore (t2_dy, dy) {
    ...
    void diagonally_make_rect_action() {
        rep(x2, N) {
            rep(y2, N) {
                if ( ( inc(x2, 2*N/5, 3*N/5+1) and inc(y2, 2*N/5, 3*N/5+1) ) ) {
                    rep(t1, 1, MAX_DIAG + 1) {
...
{
    ll before = 0, after = 0;
    int before_points = 0, after_points = 0;
    ll c = (N - 1) / 2;
    rep(x, N) {
        rep(y, N) {
            before += w(x, y, c);
            after += new_field[x][y] * w(x, y, c);
            before_points += pre_field[x][y];
            after_points += new_field[x][y];
        }
    }
    double points_ratio = (double)after_points / (double)before_points;
    // cerr << points_ratio << "\n";
