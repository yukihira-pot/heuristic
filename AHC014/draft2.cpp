...
struct Solver {
    ...
    pair<int, int> can_straightly_connect(pair<int, int> p2, pair<int, int>p3, pair<int, int> p4) 
    {
        ...
        rep(x, bottom, top+1) {
            fore (y, ys) if (used[x][y]) return cannot_connect_co;
        }
        rep(y, left, right+1) {
            fore (x, xs) if (used[x][y]) return cannot_connect_co;
        }
        rep(x, bottom+1, top) {
            fore (y, ys) if (field[x][y]) return cannot_connect_co;
        }
        rep(y, left+1, right) {
            fore (x, xs) if (field[x][y]) return cannot_connect_co;
        }
        return {x1, y1};
    }
