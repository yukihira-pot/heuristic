...
int calc_score(int N, int K, Field &field) {
    auto convert_2d_to_1d = [&](int cx, int cy) {
        return N * cx + cy;
    };
...
int main() {
    ...
    Field field(N, vector<char>(N)), field1, field2;
    ...
    int zero_cnt = 0;
    rep(i, N) rep(j, N) zero_cnt += (field[i][j] == '0');

    move_counter_original = 20 * K;
    if (zero_cnt * 10 <= N * N) {
        move_counter_original = 7 * K;
    } else if (zero_cnt * 5 <= N * N) {
        move_counter_original = 10 * K;
    } else if (zero_cnt * 3 <= N * N) {
        move_counter_original = 15 * K;
    } else {
        move_counter_original = 20 * K;
    }
    ...
    field_move(N, K, field1, 0);
    field_gather(N, K, field1, 0);
    field_connect(N, K, field1, 0);
    int res1 = calc_score(N, K, field1);
