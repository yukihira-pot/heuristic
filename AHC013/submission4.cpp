...
vector<pair<pii, pii>> field_connect2(int N, int K, Field &field, int partition_idx, vector<pair<pii, pii>> &output_connect) {
    /* 各コンピュータから右と下に向かって接続 */ 
    auto partition_vecs = generate_partition_vecs(K);
    // partition の順番 ex) {'1', '3', '2', '4'}
    vector<char> partition_vec = partition_vecs[partition_idx]; 
    ...
    rep(idx, K) {
        ...
        rep(i1, N) {
            rep(j1, N) {
                // 右
                if (field[i1][j1] == partition_vec[idx]) {
                    rep(j2, j1, N) {
                        if (can_connect(i1, j1, i1, j2)) {
                            output_connect.push_back({{i1, j1}, {i1, j2}});
                            connect(i1, j1, i1, j2);
...
int main() {
    ...
    for (int itr = 0; itr < rot; itr += incr) {
        for (int i = 0; i <= 20; i++) {
            ...
            auto res = my_solve(N, K, field1, itr);
